...
const bankNameSchema = z
  ...
  .regex(
    /^[a-zA-Z\s&.-]+$/,
    "Bank name should only contain letters, spaces, &, -, and ."
  );
...
const upiIdSchema = z
  ...
  .refine(
    ...
    "UPI ID must not exceed 50 characters"
  )
  .optional();
...
export const financialSchema = z
  .object({
    ...
    accountType: z
      ...
      .refine(
        (val) =>
          ["savings", "current", "fixed-deposit"].includes(val.toLowerCase()),
    ...
    payoutFrequency: z
      ...
      .refine(
        (val) => ["weekly", "biweekly", "monthly"].includes(val.toLowerCase()),
    ...
    consentToTerms: z
      ...
      .refine(
        (val) => val === true,
        "You must agree to the payment terms and conditions"
      ),

    consentToDataSharing: z
      .boolean()
      .refine(
        (val) => val === true,
        "You must consent to share necessary financial information for payment processing"
      ),
  })
